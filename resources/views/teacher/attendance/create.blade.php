        ...
        @if($selectedGroup)
            ...
            <form method="POST" action="{{ route('teacher.attendance.store') }}">
                ...
                <div class="mt-6 flex space-x-3">
                    ...
                    <a href="{{ route('teacher.dashboard') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-medium py-2 px-4 rounded-lg">{{ __('Cancel') }}</a>
                </div>
            </form>
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <form method="POST" action="{{ route('teacher.attendance.add-single') }}" class="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                    @csrf
                    <input type="hidden" name="dance_group_id" value="{{ $selectedGroup }}">
                    <input type="hidden" name="date" value="{{ $selectedDate }}">
                    <div>
                        <label class="block text-xs text-gray-500">Uczeń</label>
                        <select name="student_id" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 border p-2">
