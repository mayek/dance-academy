...
<div class="px-4 sm:px-0">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('My Events') }}</h1>
        <a href="{{ route('teacher.events.create') }}" class="bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium py-2 px-4 rounded-lg">{{ __('+ Add Event') }}</a>
    </div>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm space-x-2">
                        ...
                        <a href="{{ route('teacher.events.assign', $event) }}" class="text-indigo-600 hover:text-indigo-800 inline-flex align-middle">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
                        </a>
                        <a href="{{ route('teacher.events.edit', $event) }}" class="text-emerald-600 hover:text-emerald-800 inline-flex align-middle">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        </a>
                        <form method="POST" action="{{ route('teacher.events.destroy', $event) }}" class="inline" onsubmit="return confirm('{{ __('Are you sure?') }}')">
                            @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">{{ __('No events found.') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div id="event-pass-modal" class="hidden fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
    <div class="flex items-end sm:items-center justify-center min-h-screen p-4 sm:p-6">
        ...
        <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <form method="POST" action="{{ route('teacher.events.buy-pass') }}" class="px-5 py-4">
                @csrf
