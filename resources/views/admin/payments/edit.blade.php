        ...
        <form method="POST" action="{{ route('admin.payments.update', $payment) }}" id="paymentForm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div class="relative">
                    ...
                    <div id="student_clear" class="absolute right-2 top-8 cursor-pointer text-gray-400 hover:text-gray-600 text-sm">clear</div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <select name="pass_type" id="pass_type" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm border p-2">
                        <option value="monthly" {{ old('pass_type', $payment->pass_type) == 'monthly' ? 'selected' : '' }}>Monthly</option>
                        <option value="single" {{ old('pass_type', $payment->pass_type) == 'single' ? 'selected' : '' }}>Single Class</option>
                    </select>
                </div>
                <div>
            ...
            <div class="flex space-x-3">
                <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white font-medium py-2 px-4 rounded-lg">Update Payment</button>
