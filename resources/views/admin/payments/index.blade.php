...
<div class="px-4 sm:px-0">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.payments.create') }}" class="bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium py-2 px-4 rounded-lg">+ Record Payment</a>
    </div>

    <div class="bg-white shadow rounded-lg p-4 mb-6">
        <form method="GET" action="{{ route('admin.payments.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
                <label for="student_id" class="block text-xs font-medium text-gray-500 mb-1">Student</label>
            ...
            <div>
                <label for="dance_group_id" class="block text-xs font-medium text-gray-500 mb-1">Group</label>
            ...
            <div>
                <label for="pass_type" class="block text-xs font-medium text-gray-500 mb-1">Pass Type</label>
                <select name="pass_type" id="pass_type" class="w-full rounded-md border-gray-300 text-sm border p-2">
                    <option value="">All types</option>
                    <option value="monthly" {{ request('pass_type') == 'monthly' ? 'selected' : '' }}>Monthly</option>
                    <option value="single" {{ request('pass_type') == 'single' ? 'selected' : '' }}>Single Class</option>
                </select>
            </div>
            <div>
            ...
            <div class="flex items-end">
                <button type="submit" class="w-full bg-gray-600 hover:bg-gray-700 text-white text-sm font-medium py-2 px-4 rounded-lg">Filter</button>
            </div>
        </form>
    </div>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm space-x-2">
                        <a href="{{ route('admin.payments.edit', $payment) }}" class="text-emerald-600 hover:text-emerald-800">Edit</a>
                ...
                @empty
                <tr><td colspan="8" class="px-6 py-4 text-center text-sm text-gray-500">No payments found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
