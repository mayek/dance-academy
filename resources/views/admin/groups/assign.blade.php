@extends('layouts.app')
@section('title', 'Assign Students to ' . $group->name)

@section('content')
<div class="px-4 sm:px-0 max-w-2xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-2">Assign Students</h1>
    <p class="text-gray-600 mb-6">Group: <strong>{{ $group->name }}</strong> ({{ $group->category->name }}) &mdash; Teacher: {{ $group->teacher->name }}</p>

    <div class="bg-white shadow rounded-lg p-6">
        <form method="POST" action="{{ route('admin.groups.update-students', $group) }}">
            @csrf @method('PUT')
            ...
            <div class="flex space-x-3">
                ...
                <a href="{{ route('admin.groups.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-medium py-2 px-4 rounded-lg">Cancel</a>
            </div>
        </form>
    </div>
</div>
