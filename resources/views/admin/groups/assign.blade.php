@extends('layouts.app')
@section('title', __('Assign Students to ') . $group->name)

@section('content')
<div class="px-4 sm:px-0 max-w-2xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-2">{{ __('Assign Students') }}</h1>
    <p class="text-gray-600 mb-6">{{ __('Group') }}: <strong>{{ $group->name }}</strong> ({{ $group->category->name }}) &mdash; {{ __('Teacher') }}: {{ $group->teacher->name }}</p>
