    ...
    <div class="bg-white shadow rounded-lg p-6">
        <form method="POST" action="{{ route('admin.groups.update', $group) }}">
            ...
            <div class="mb-6">
                ...
                <input type="hidden" name="class_times" id="class_times" value="{{ json_encode($ct) }}">
                <div class="space-y-2" id="class_times_fields">
                    @foreach(['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'] as $i => $day)
                    @php
                        $dayNum = $i + 1;
                        $entry = collect($ct)->firstWhere('day', $dayNum);
                    @endphp
                    <div class="flex items-center gap-3 p-2 rounded hover:bg-gray-50">
                        <input type="checkbox" data-day="{{ $dayNum }}" class="day-checkbox rounded border-gray-300 text-orange-600 focus:ring-orange-500" {{ $entry ? 'checked' : '' }}>
                        <span class="text-sm text-gray-700 w-24">{{ __($day) }}</span>
                        <input type="time" class="start-time rounded-md border-gray-300 text-sm border p-1.5" value="{{ $entry['start'] ?? '18:00' }}" {{ $entry ? '' : 'disabled' }}>
                        <span class="text-sm text-gray-400">&ndash;</span>
                        <input type="time" class="end-time rounded-md border-gray-300 text-sm border p-1.5" value="{{ $entry['end'] ?? '19:30' }}" {{ $entry ? '' : 'disabled' }}>
                    </div>
                    @endforeach
                </div>
