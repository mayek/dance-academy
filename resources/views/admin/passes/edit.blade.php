@extends('layouts.app')
@section('title', __('Edit Pass'))

@section('content')
<div class="px-4 sm:px-0 max-w-2xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">{{ __('Edit Pass') }}</h1>
    <div class="bg-white shadow rounded-lg p-6">
        <form method="POST" action="{{ route('admin.passes.update', $pass) }}">
            @csrf @method('PUT')
            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-gray-700">{{ __('Name') }}</label>
                <input type="text" name="name" id="name" value="{{ old('name', $pass->name) }}"
            ...
            <div class="mb-4">
                ...
                <select name="duration" id="duration" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm border p-2">
                    <option value="single" {{ old('duration', $pass->isSingle() ? 'single' : $pass->duration_months) == 'single' ? 'selected' : '' }}>{{ __('Single Pass') }}</option>
                    <option value="1" {{ old('duration', $pass->isSingle() ? 'single' : $pass->duration_months) == '1' ? 'selected' : '' }}>{{ __('1 month') }}</option>
                    <option value="2" {{ old('duration', $pass->isSingle() ? 'single' : $pass->duration_months) == '2' ? 'selected' : '' }}>{{ __('2 months') }}</option>
                    <option value="3" {{ old('duration', $pass->isSingle() ? 'single' : $pass->duration_months) == '3' ? 'selected' : '' }}>{{ __('3 months') }}</option>
                    <option value="6" {{ old('duration', $pass->isSingle() ? 'single' : $pass->duration_months) == '6' ? 'selected' : '' }}>{{ __('6 months') }}</option>
                    <option value="12" {{ old('duration', $pass->isSingle() ? 'single' : $pass->duration_months) == '12' ? 'selected' : '' }}>{{ __('12 months') }}</option>
                </select>
            ...
            <div class="mb-6">
                ...
                <input type="number" name="price" id="price" step="0.01" min="0" value="{{ old('price', $pass->price) }}" required
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm border p-2">
            ...
            <div class="mb-6">
                ...
                <p class="mt-1 text-xs text-gray-500">{{ __('Total hours for the entire pass duration. Leave empty to calculate automatically from the schedule.') }}</p>
            </div>
